...
from rtransformer.masked_transformer import MTransformer
from rtransformer.beam_search import BeamSearch
...
class Translator(object):
    ...
    def __init__(self, opt, checkpoint, model=None):
        ...
        if model is None:
            ...
            model = TextKG(self.model_config).to(self.device)
    ...
    def translate_batch_single_sentence_greedy(self, input_ids, input_ids2,video_features, region_features,input_masks, input_masks2,token_type_ids,token_type_ids2,kg_masks,l,model,start_idx=RCDataset.BOS, unk_idx=RCDataset.UNK):
        ...
        input_ids[:, -22:] = 0.
        input_masks[:, -22:] = 0.
        input_ids2[:, 98*5+2+max_asr_len:98*5+2+max_asr_len+22] = 0.
        input_masks2[:, 98*5+2+max_asr_len:98*5+2+max_asr_len+22] = 0.
        ...
        for dec_idx in range(max_v_len, max_v_len + max_t_len):
            input_ids[:, dec_idx] = next_symbols
            input_ids2[:,dec_idx-max_v_len+98*5+2+max_asr_len] = next_symbols
            input_masks[:, dec_idx] = 1
            input_masks2[:,dec_idx-max_v_len+98*5+2+max_asr_len] = 1
            # if dec_idx < max_v_len + 5:
            #     logger.info("prev_ms {} {}".format(type(prev_ms[0]), prev_ms[0]))
            _, pred_scores = model.forward(input_ids, input_ids2, video_features, region_features, input_masks, input_masks2, token_type_ids, token_type_ids2, l, None, None, kg_masks)
            # suppress unk token; (N, L, vocab_size)
            pred_scores[:, :, unk_idx] = -1e10
            # next_words = pred_scores.max(2)[1][:, dec_idx]
        ...
        if use_beam:
            ...
            if recurrent:
                ...
                if untied or mtrans:
                    ...
                    input_ids_list,input_ids_list2, video_features_list, region_features_list,input_masks_list, input_masks_list2,token_type_ids_list,token_type_ids_list2,kg_masks,l_mask = model_inputs
                    return self.translate_batch_single_sentence_greedy(
                        input_ids_list,input_ids_list2, video_features_list, region_features_list,input_masks_list, input_masks_list2,token_type_ids_list,token_type_ids_list2,kg_masks,l_mask,
                        self.model)
